...
static error_t _kalman_calc_sigmas(kalman_context_t* context) {
    ...
    size_t sigma_plus_minus_count = (context->chi_km1->cols - 1) / 2;
    for (size_t n = 1; n < sigma_plus_minus_count + 1; n++) {
        if (E_OK != m_copy_column(context->x_hat, 0, context->chi_km1, n)) {
            return E_VAL;
        }

        if (E_OK != m_add_scaled_column(context->sqrt_P_km1, n - 1, context->eta, context->chi_km1, n)) {
            return E_VAL;
        }

        if (E_OK != m_copy_column(context->x_hat, 0, context->chi_km1, sigma_plus_minus_count + n)) {
            return E_VAL;
        }
